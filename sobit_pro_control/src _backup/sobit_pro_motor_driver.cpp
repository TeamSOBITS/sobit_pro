...
bool SobitProMotorDriver::setTorque(uint8_t id, bool onoff){
    ...
    if(dxl_comm_result != COMM_SUCCESS){
        ...
        if(onoff == true)  std::cout << "Dynamixel ID: " << int(id) << " has been successfully connected!\n"  << std::endl;
        if(onoff == false) std::cout << "Dynamixel ID: " << int(id) << " has been successfully disconnected!" << std::endl;
    }
...
int32_t SobitProMotorDriver::feedbackSteer(uint8_t id){
  ...
  steer_curt_angle_ = groupSyncReadPosition_->getData(id, ADDR_X_PRESENT_POSITION, LEN_X_PRESENT_POSITION);
