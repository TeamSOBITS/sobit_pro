...
public:
  ...
  static constexpr double DXL_MOVING_STATUS_THRESHOLD = 0.2;                                 // Dynamixel moving status threshold
  static constexpr double STEER_PUBLISH_EPSILON       = 0.05;                                // Minimum change to consider as new command [rad]
  ...
  SobitProControl(rclcpp::Node* node) : node_(node) {
    ...
    steer_fl_goal_pos = 0.0;
    steer_fr_goal_pos = 0.0;
    steer_bl_goal_pos = 0.0;
    steer_br_goal_pos = 0.0;

    wheel_fl_goal_vel = 0.0;
    wheel_fr_goal_vel = 0.0;
    wheel_bl_goal_vel = 0.0;
    wheel_br_goal_vel = 0.0;
  }
  // Destructor
  ~SobitProControl() {
